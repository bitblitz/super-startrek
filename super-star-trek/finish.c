...
void kaboom(void) {
	stars();
	if (ship_name==IH_ENTERPRISE) prouts("***");
	prouts("********* Entropy of ");
	crmshp();
	prouts(" maximized *********");
	skip(1);
	...
	if (currentq_num_enemies != 0) {
		double whammo = 25.0 * ship_energy;
		int l=1;
		while (l <= currentq_num_enemies) {
...
void finish(FINTYPE ifin) {
	...
	switch (ifin) {
		case FWON: // Game has been won
			...
			if (alive) {
				...
				badpt = 5.*g_d.killed_stars + num_casualties + 10.*g_d.killed_planets +
						45.*game_num_help_calls+100.*g_d.killed_bases;
				if (ship_name == IH_FAERIE_QUEEN) badpt += 100.0;
				else if (ship_name == 0) badpt += 200.0;
				if (badpt < 100.0) badpt = 0.0;	// Close enough!
				if (g_d.stardate-game_initial_stardate < 5.0 ||
					// killsPerDate >= RateMax
					(g_d.killed_klingons+g_d.killed_commanders+g_d.killed_supercommanders)/(g_d.stardate-game_initial_stardate) >=
					0.1*game_skill*(game_skill+1.0) + 0.1 + 0.008*badpt) {
					skip(1);
		...
		case FDEPLETE: // Federation Resources Depleted
			...
			if (g_d.remaining_klingons*3.0 > initial_klingons) {
				prout("aquitted.");
				skip(1);
				prout("LIVE LONG AND PROSPER.");
			}
	...
	if (ship_name==IH_FAERIE_QUEEN) ship_name= 0;
	else if (ship_name == IH_ENTERPRISE) ship_name = IH_FAERIE_QUEEN;
	alive = 0;
	if (g_d.remaining_klingons != 0) {
		double goodies = g_d.remaining_resources/game_initial_resources;
		double baddies = (g_d.remaining_klingons + 2.0*g_d.remaining_commanders)/(initial_klingons+2.0*initial_commanders);
...
void score(int inGame) {
	double timused = g_d.stardate - game_initial_stardate;
    int ithperd, iwon, klship;
    int dremaining_romulans = g_d.remaining_romulans; // Leave global value alone

    if (!inGame) pause(0);

	iskill = game_skill;
	if ((timused == 0 || g_d.remaining_klingons != 0) && timused < 5.0) timused = 5.0;
	perdate = (g_d.killed_commanders + g_d.killed_klingons + g_d.killed_supercommanders)/timused;
	ithperd = (int)(500*perdate + 0.5);
	iwon = 0;
	if (gamewon) iwon = 100*game_skill;
	if (ship_name == IH_ENTERPRISE) klship = 0;
	else if (ship_name == IH_FAERIE_QUEEN) klship = 1;
	else klship = 2;
	if (gamewon == 0 || inGame) dremaining_romulans = 0; // None captured if no win or if still in the game
	iscore = 10*g_d.killed_klingons + 50*g_d.killed_commanders + ithperd + iwon
			 - 100*g_d.killed_bases - 100*klship - 45*game_num_help_calls -5*g_d.killed_stars - num_casualties
		 + 20*g_d.killed_romulans + 200*g_d.killed_supercommanders - 10*g_d.killed_planets + dremaining_romulans;
#ifdef CLOAKING
	iscore -= 100*num_cloak_violations;
#endif
#ifdef CAPTURE
	iscore += 3*captured_klingons;
#endif
	if (alive == 0) iscore -= 200;
	skip(2);
    if (inGame) prout("Your score so far --");
    else prout("Your score --");
	if (g_d.killed_romulans)
		printf(g_d.killed_romulans> 1 ? "%6d Romulan ships destroyed            %5d\n" : "%6d Romulan ship destroyed             %5d\n",
			   g_d.killed_romulans, 20*g_d.killed_romulans);
	if (dremaining_romulans)
		printf(dremaining_romulans > 1 ? "%6d Romulan ships captured             %5d\n" : "%6d Romulan ship captured              %5d\n",
			   dremaining_romulans, dremaining_romulans);
	if (g_d.killed_klingons)
		printf(g_d.killed_klingons > 1 ? "%6d ordinary Klingon ships destroyed   %5d\n" : "%6d ordinary Klingon ship destroyed    %5d\n",
			   g_d.killed_klingons,  10*g_d.killed_klingons);
	if (g_d.killed_commanders)
		printf(g_d.killed_commanders > 1 ? "%6d Klingon Commander ships destroyed  %5d\n" : "%6d Klingon Commander ship destroyed   %5d\n",
			   g_d.killed_commanders, 50*g_d.killed_commanders);
	if (g_d.killed_supercommanders)
		printf("%6d Super-Commander ship destroyed     %5d\n",
			   g_d.killed_supercommanders, 200*g_d.killed_supercommanders);
	if (ithperd)
		printf("%6.2f Klingons per stardate              %5d\n",
			   perdate, ithperd);
#ifdef CAPTURE
	if (captured_klingons)
		printf(captured_klingons > 1 ? "%6d Klingons captured                  %5d\n" : "%6d Klingon captured                   %5d\n",
		        captured_klingons, 3*captured_klingons);
#endif
	if (g_d.killed_stars)
		printf(g_d.killed_stars > 1 ? "%6d stars destroyed by your action     %5d\n" : "%6d star destroyed by your action      %5d\n",
			   g_d.killed_stars, -5*g_d.killed_stars);
	if (g_d.killed_planets)
		printf(g_d.killed_planets > 1 ? "%6d planets destroyed by your action   %5d\n" : "%6d planet destroyed by your action    %5d\n",
			   g_d.killed_planets, -10*g_d.killed_planets);
	if (g_d.killed_bases)
		printf(g_d.killed_bases > 1 ? "%6d bases destroyed by your action     %5d\n" : "%6d base destroyed by your action      %5d\n",
			   g_d.killed_bases, -100*g_d.killed_bases);
	if (game_num_help_calls)
		printf(game_num_help_calls > 1 ? "%6d calls for help from starbase       %5d\n" : "%6d call for help from starbase        %5d\n",
			   game_num_help_calls, -45*game_num_help_calls);
	if (num_casualties)
		printf(num_casualties > 1 ? "%6d casualties incurred                %5d\n" : "%6d casualty incurred                  %5d\n",
			   num_casualties, -num_casualties);
	if (klship)
		printf(klship > 1 ? "%6d ships lost or destroyed            %5d\n" : "%6d ship lost or destroyed             %5d\n",
			   klship, -100*klship);
#ifdef CLOAKING
	if (num_cloak_violations>0)
		printf(num_cloak_violations > 1 ? "%6d Treaty of Algeron violations       %5d\n" : "%6d Treaty of Algeron violation        %5d\n",
		       num_cloak_violations, -100*num_cloak_violations);
#endif
	if (alive==0)
    ...
    if (inGame && game_skill < SGOOD) printf("REMEMBER--The score doesn't really matter until the mission is accomplished!\n");
}
